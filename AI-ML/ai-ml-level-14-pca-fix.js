(function () {
    ...
    function start() {
        ...
        if (!canvas || canvas.dataset.cbPcaRenderer === "active") return;
        ...
        const colors = ["#22d3ee", "#fbbf24", "#a78bfa", "#34d399"];
        ...
        function calculate() {
            const source = datasets[datasetInput.value] || datasets.correlated;
            ...
            if (Math.abs(b) > 1e-10) {
                pc1 = [b, eigenvalues[0] - a];
            } else {
                pc1 = a >= d ? [1, 0] : [0, 1];
            }

            const vectorLength = Math.hypot(pc1[0], pc1[1]) || 1;
            ...
            const retained = Number(componentInput.value);
            ...
            const reconstructedPrepared = scores.map(function (score) {
                ...
                for (
                    ...
                    output[0] +=
                        score[component] * vectors[component][0];

                    output[1] +=
                        score[component] * vectors[component][1];
                }
            ...
            const error = raw.reduce(function (sum, row, index) {
                return sum +
            ...
            return {
                ...
                retained: retained,
                rmse: Math.sqrt(error / (raw.length * 2))
            };
        }
        ...
        function draw() {
            ...
            for (let index = 0; index <= 8; index += 1) {
                const x =
                    padding +
                    (width - padding * 2) * index / 8;

                const y =
                    padding +
                    (height - padding * 2) * index / 8;
            ...
            if (step >= 4) {
                ...
                result.vectors.forEach(function (vector, index) {
            ...
            points.forEach(function (row, index) {
                ...
                context.fillStyle =
                    colors[result.labels[index] % colors.length];
        ...
        function render() {
            ...
            const cumulative = result.ratios
            ...
            get("pcaVariance").textContent =
                step >= 4
                    ? format(result.ratios[0] * 100, 1) + "%"
                    : "—";

            get("pcaCumulative").textContent =
                step >= 5
                    ? format(cumulative * 100, 1) + "%"
                    : "—";
            ...
            get("pcaLoadings").innerHTML =
                step >= 4
                    ? result.vectors.map(function (vector, index) {
                        return (
                            "<article>" +
                                "<span>PC" +
                                    (index + 1) +
            ...
            get("pcaVerdict").textContent = message[0];
            get("pcaExplanation").textContent = message[1];
            get("pcaNextCheck").textContent = message[2];
        ...
        autoButton.addEventListener("click", function () {
            if (step >= phases.length || timer !== null) {
                return;
            }
        ...
        canvas.dataset.cbPcaRenderer = "active";
