(function () {
    ...
    function initProblems() {
        ...
        const requirements = [
            [["step = size - overlap", "step=size-overlap"], ["range("], ["words[i:i + size]", "words[i:i+size]"], ["return"]],
            [["enumerate("], ["tokens.count("], ["idf[i]"], ["return"]],
            [["sqrt("], ["zip("], ["denom"], ["sorted("], ["reverse=true", "reverse = true"], ["return"]],
            [["set(args)"], ["isinstance("], ["not isinstance(amount, bool)", "not isinstance(amount,bool)"], ["amount <= maximum", "amount<=maximum"], ["in allowed"]],
            [["for _ in range(max_steps)", "for _ in range (max_steps)"], ["policy("], ["execute("], ["trace.append("], ["budget_exhausted"], ["return"]]
        ];
        const progress = readProgress();
        const legacySolved = progress.solved && typeof progress.solved === "object" ? Object.keys(progress.solved).filter(function (key) { return progress.solved[key]; }).map(Number) : [];
        const solved = new Set(Array.isArray(progress.solvedProblems) ? progress.solvedProblems : legacySolved);
        const scores = progress.problemScores && typeof progress.problemScores === "object" ? progress.problemScores : {};
        const revealed = new Set();
        list.innerHTML = PROBLEMS.map(function (problem, index) {
            const number = index + 1;
            const parts = problem.example.split("\nOutput:");
            const sampleInput = parts[0].replace(/^Input:\s*/, "");
            const expected = parts.length > 1 ? parts.slice(1).join("\nOutput:").trim() : "See the expected result in the prompt.";
            return '<article class="aiml-problem-card' + (solved.has(index) ? " is-solved" : "") + '" data-problem="' + index + '"><div class="aiml-problem-head"><span class="aiml-problem-number">' + String(number).padStart(2, "0") + '</span><div><h3>' + number + ". " + escapeHtml(problem.title) + '</h3><p>' + escapeHtml(problem.prompt) + '</p></div></div><div class="aiml-problem-data"><span><strong>Difficulty:</strong> ' + escapeHtml(problem.difficulty) + '</span><span><strong>Sample input:</strong> ' + escapeHtml(sampleInput) + '</span><span><strong>Expected output:</strong> <code>' + escapeHtml(expected) + '</code></span></div><div class="aiml-problem-actions"><button type="button" class="primary" data-action="workspace">💻 Solve It Yourself</button><button type="button" class="hint" data-action="hint">Hint</button><button type="button" data-action="solution">Show Program</button></div><div class="aiml-problem-reveal" data-panel="hint" hidden><strong>Hint</strong><p>' + escapeHtml(problem.hint) + '</p></div><div class="aiml-problem-reveal" data-panel="solution" hidden><strong>Model program</strong><pre><code>' + escapeHtml(problem.solution) + '</code></pre></div><div class="aiml-workspace" data-panel="workspace" hidden><label for="problemCode' + index + '">Your Python code</label><textarea id="problemCode' + index + '" spellcheck="false">' + escapeHtml(problem.starter) + '</textarea><div class="aiml-workspace-row"><button type="button" data-action="check">Check Answer</button><button type="button" data-action="reset">Reset</button><span class="aiml-check-result" data-result>Write your solution, then check its structure.</span></div></div></article>';
        }).join("");

        function updateSummary() {
            const total = Object.keys(scores).reduce(function (value, key) { return value + Number(scores[key] || 0); }, 0);
        ...
        list.addEventListener("click", function (event) {
            ...
            if (action === "workspace" || action === "hint" || action === "solution") {
                ...
                if (action === "solution") { if (opening) revealed.add(index); button.textContent = opening ? "Hide Program" : "Show Program"; }
                return;
            }
            ...
            if (action === "check") {
                ...
                const missing = requirements[index].filter(function (alternatives) {
                    return !alternatives.some(function (token) { return normalized.indexOf(token.toLowerCase()) !== -1; });
                });
                ...
                if (missing.length) {
                    result.className = "aiml-check-result error";
                    result.textContent = "Not complete yet. Recheck the required RAG or agent logic.";
                    return;
                }
                ...
                writeProgress({ solvedProblems: Array.from(solved), problemScores: scores });
                updateSummary();
            }
        });
        updateSummary();
    ...
    function initQuiz() {
        ...
        list.innerHTML = QUIZ.map(function (item, index) {
            return '<article class="aiml-quiz-question" data-quiz-question="' + index + '"><strong>' + (index + 1) + ". " + escapeHtml(item.q) + '</strong><div class="aiml-quiz-options">' + item.options.map(function (option, optionIndex) {
                const id = "quiz-twenty-two-" + index + "-" + optionIndex;
                return '<label class="aiml-quiz-option" for="' + id + '"><input type="radio" id="' + id + '" name="quiz-twenty-two-' + index + '" value="' + optionIndex + '"><span>' + String.fromCharCode(65 + optionIndex) + ". " + escapeHtml(option) + "</span></label>";
        ...
        checkButton.addEventListener("click", function () {
            ...
            QUIZ.forEach(function (item, index) {
                const card = list.querySelector('[data-quiz-question="' + index + '"]');
                const selected = card.querySelector('input[type="radio"]:checked');
                const options = Array.from(card.querySelectorAll(".aiml-quiz-option"));
                const explanation = card.querySelector(".aiml-quiz-explanation");
                ...
                explanation.innerHTML = '<strong>Your answer: ' + (selected ? escapeHtml(item.options[Number(selected.value)]) : "Not attempted") + '</strong><br><strong>Correct answer: ' + escapeHtml(item.options[item.answer]) + "</strong><br>" + escapeHtml(item.why);
            });
            get("quizScore").textContent = correct + " / " + QUIZ.length + " correct" + (answered < QUIZ.length ? " • " + (QUIZ.length - answered) + " not attempted" : "");
        ...
        resetButton.addEventListener("click", function () {
            list.querySelectorAll('input[type="radio"]').forEach(function (input) { input.checked = false; });
            list.querySelectorAll(".aiml-quiz-option").forEach(function (option) { option.classList.remove("is-selected", "is-correct", "is-wrong"); });
