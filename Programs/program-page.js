...
(function () {
  ...
  function downloadSource() {
    const blob = new Blob([source], { type: config.language === "python" ? "text/x-python;charset=utf-8" : "text/x-c;charset=utf-8" });
  ...
  function explainLine(line) {
    ...
    if (config.language === "python") {
      if (/^(?:from\s+\S+\s+)?import\s+/.test(trimmed)) return "This statement imports reusable Python functionality.";
      if (/^(?:async\s+)?def\s+/.test(trimmed)) return "This line defines a Python function and its parameters.";
      if (/^class\s+/.test(trimmed)) return "This line creates a class that groups data and behavior.";
      if (/^(?:if|elif)\b/.test(trimmed)) return "This condition selects a branch when its expression is true.";
      if (/^else:/.test(trimmed)) return "This alternate branch runs when the earlier conditions are false.";
      if (/^(?:for|while)\b/.test(trimmed)) return "This loop repeats the indented block while iterating or testing a condition.";
      if (/^try:/.test(trimmed)) return "This block contains an operation that may raise an exception.";
      if (/^except\b/.test(trimmed)) return "This handler responds safely to the specified exception.";
      if (/^with\s+/.test(trimmed)) return "This context manager acquires a resource and closes it automatically.";
      if (/^return\b/.test(trimmed)) return "The function sends this value back to its caller.";
      if (/^yield\b/.test(trimmed)) return "The generator produces one value and pauses until the next request.";
      if (/\bprint\s*\(/.test(trimmed)) return "This statement displays the current result.";
      if (/\binput\s*\(/.test(trimmed)) return "This statement reads text entered by the user.";
      if (/=/.test(trimmed)) return "This statement calculates or assigns a Python value.";
      return "Read this Python statement with its indentation and identify how it changes the data or control flow.";
    }
