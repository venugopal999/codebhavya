...
const programs = moduleFiles.flatMap((file) => require(path.join(moduleDirectory, file)));
...
function validatePrograms() {
  ...
  if (programs.length !== 150) {
    throw new Error(`Expected exactly 150 programs, received ${programs.length}.`);
  }
...
function topicSidebar(program, previous, next) {
  ...
  links.push(`<a href="index.html">View all 150 programs</a>`);
  ...
  return `<aside class="example-sidebar">
    <h2 class="sidebar-title">C Program Library</h2>
...
function pageNavigation(previous, next) {
  return `<nav class="program-navigation" aria-label="Program navigation">
    ${previous ? `<a href="${previous.slug}.html"><span>← Previous</span><strong>${escapeHtml(previous.title)}</strong></a>` : `<a href="index.html"><span>← Library</span><strong>All C programs</strong></a>`}
...
function renderPage(program, index) {
  const previous = programs[index - 1] || null;
  const next = programs[index + 1] || null;
...
<head>
  ...
  <title>C Program to ${escapeHtml(program.title)} | CodeBhavya</title>
...
<body>
  ...
  <main class="program-main" id="programContent">
    <nav class="breadcrumbs" aria-label="Breadcrumb"><a href="../index.html">Home</a><span>›</span><a href="index.html">Program Library</a><span>›</span><span>C Programming</span><span>›</span><span aria-current="page">${escapeHtml(program.title)}</span></nav>
    <div class="program-page-layout">
      ${topicSidebar(program, previous, next)}
      <article class="program-article">
        <header class="program-hero">
          <p class="eyebrow">C PROGRAM • ${escapeHtml(program.topic.toUpperCase())}</p>
        ...
        <section class="content-card" id="programCode">
          <p class="section-kicker">COMPLETE C PROGRAM</p>
          <h2>Code, copy and run</h2>
          <div class="code-window"><div class="code-toolbar"><strong>${program.slug}.c</strong><div class="code-actions"><button id="copyCode" type="button">Copy code</button><button id="downloadCode" type="button">Download .c</button><a id="copyOpenCompiler" href="../Online-Compiler/index.html">Open in compiler</a></div></div><pre class="source-code" id="sourceCode" tabindex="0">${escapeHtml(program.source)}</pre></div>
        </section>
...
programs.forEach((program, index) => {
  fs.writeFileSync(path.join(sourceDirectory, `${program.slug}.c`), `${program.source}\n`);
  if (!program.customPage) {
    fs.writeFileSync(path.join(root, `${program.slug}.html`), renderPage(program, index));
    generatedPages++;
  }
});

console.log(`Prepared ${programs.length} C programs: ${generatedPages} standard pages and ${programs.length - generatedPages} advanced debugger pages.`);
