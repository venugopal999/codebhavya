...
const javaSourceDirectory = path.join(root, "java-source");
...
const javaPrograms = loadModules(path.join(root, "java-library"));
const programs = [...cPrograms, ...dsaPrograms, ...adsPrograms, ...pythonPrograms, ...aiMlPrograms, ...javaPrograms];
...
function validatePrograms() {
  ...
  if (cPrograms.length !== 200 || dsaPrograms.length !== 106 || adsPrograms.length !== 112 || pythonPrograms.length !== 150 || aiMlPrograms.length !== 130 || javaPrograms.length !== 200) {
    throw new Error(`Expected 200 C, 106 DSA, 112 ADS, 150 Python, 130 AI/ML and 200 Java programs; received ${cPrograms.length}, ${dsaPrograms.length}, ${adsPrograms.length}, ${pythonPrograms.length}, ${aiMlPrograms.length} and ${javaPrograms.length}.`);
  }
...
fs.mkdirSync(javaSourceDirectory, { recursive: true });
writeCatalogData();
writeSitemap();
