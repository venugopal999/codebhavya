(async()=>{
 ...
 async function load(){
   ...
   <p>${quiz.question_count} questions · ${Math.round(quiz.duration_seconds/60)} minutes · Joined: <strong>${quiz.joined_count}</strong></p>
   ...
   if($("startBtn"))$("startBtn").onclick=()=>{if(confirm("Start the quiz for all joined students now?"))action("quiz_admin_start_v1")};
 ...
 async function loadParticipants(){
   const {data}=await client.rpc("quiz_admin_participants_v1",{p_quiz_id:id});
   $("participants").innerHTML=(data||[]).map(p=>`<tr><td>${esc(p.student_label)}</td><td>${esc(localDate(p.joined_at))}</td><td>${esc(p.status)}</td><td>${p.score==null?"—":p.score}</td></tr>`).join("")||`<tr><td colspan="4">No students joined yet.</td></tr>`;
 }
 await load();
