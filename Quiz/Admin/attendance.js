(async()=>{
  ...
  const {data,error}=await client.rpc('quiz_admin_attendance_v10',{p_quiz_id:id});
  ...
  panel.innerHTML=`<div class="section-head"><div><h2>Section Attendance</h2><p class="muted">Based on the roster saved when sections were assigned.</p></div><button class="btn teal" id="exportAttendance">Export Attendance CSV</button></div>
    ...
    <div class="tablewrap"><table class="table"><thead><tr><th>Student</th><th>Class / Section</th><th>Status</th><th>Score</th><th>Joined</th><th>Details</th></tr></thead><tbody id="attendanceRows"></tbody></table></div>`;
  function render(){
    const selected=$('attendanceSection').value;
    const students=(data.students||[]).filter(s=>!selected||s.section_id===selected);
    $('attendanceRows').innerHTML=students.map(s=>`<tr><td><strong>${esc(s.student_name||s.email)}</strong><br><span class="muted">${esc(s.email)}</span></td><td>${esc(s.class_name)} · ${esc(s.section_name)}</td><td>${esc(s.status)}</td><td>${s.score==null?'—':Number(s.score)}</td><td>${esc(localDate(s.joined_at))}</td><td>${s.attempt_id?`<a class="btn ghost detail-btn" href="student-result.html?attempt=${encodeURIComponent(s.attempt_id)}">View</a>`:'—'}</td></tr>`).join('')||'<tr><td colspan="6">No students in this selection.</td></tr>';
  }
  $('attendanceSection').onchange=render;render();
  $('exportAttendance').onclick=()=>{
    const headers=['Course','Class','Section','Student','Email','Status','Score','Joined','Submitted'];
    const rows=(data.students||[]).map(s=>[data.sections.find(x=>x.section_id===s.section_id)?.course||'',s.class_name,s.section_name,s.student_name,s.email,s.status,s.score??'',s.joined_at||'',s.submitted_at||'']);
