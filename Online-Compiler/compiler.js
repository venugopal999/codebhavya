...
(() => {
  ...
  dialogStyles.href=new URL('compiler-dialog.css?v=33',document.currentScript.src).href;
  document.head.append(dialogStyles);
  const PUBLIC_JUDGE0_ENDPOINT =
    "https://ce.judge0.com/submissions?base64_encoded=false&wait=true";
  const INCOMING_PROGRAM_KEY = "codebhavya-compiler:incoming-program";
...
`,
  ...
  function takeIncomingProgram() {
    let raw = null;

    try {
      raw = localStorage.getItem(INCOMING_PROGRAM_KEY);
      localStorage.removeItem(INCOMING_PROGRAM_KEY);
    } catch (error) {
      return null;
    }
    ...
    try {
      ...
      if (!incoming
        || !Object.prototype.hasOwnProperty.call(languages, incoming.language)
        || typeof incoming.source !== "string"
        || !incoming.source.trim()
        || incoming.source.length > 65000) {
        return null;
      }
      ...
      return {
        ...
        stdin: typeof incoming.stdin === "string" ? incoming.stdin.slice(0, 16000) : ""
      };
    } catch (error) {
      return null;
    }
  ...
  function openInputRequest() {
    if(!elements.inputDialogForm.querySelector('.dialog-scroll-content')){
      const scroll=document.createElement('div');scroll.className='dialog-scroll-content';
      const heading=elements.inputDialogForm.querySelector('.dialog-heading');
      const actions=elements.inputDialogForm.querySelector('.dialog-actions');
      [...elements.inputDialogForm.children].forEach(child=>{if(child!==heading&&child!==actions)scroll.append(child);});
      elements.inputDialogForm.insertBefore(scroll,actions);
      elements.cancelInputDialog.textContent='← Back to code';
    }
  ...
  function loadLanguage(language, incomingProgram = null) {
    ...
    elements.stdin.value = incomingProgram ? incomingProgram.stdin : config.input;
    setSource(incomingProgram ? incomingProgram.source : (savedSource ?? config.starter));

    if (incomingProgram) {
      writeStored(sourceKey(language), incomingProgram.source);
    }
  ...
  function bindEvents() {
    ...
    elements.cancelInputDialog.addEventListener("click", () => {
      elements.stdin.value=elements.dialogInput.value;
      elements.inputDialog.close();
      activatePanel('editorPanel');
    });
    elements.inputDialog.addEventListener('cancel',()=>{
      elements.stdin.value=elements.dialogInput.value;
    });
  ...
  const incomingProgram = takeIncomingProgram();

  if (incomingProgram) {
    elements.language.value = incomingProgram.language;
    loadLanguage(incomingProgram.language, incomingProgram);
    showToast("Selected program loaded in the compiler.");
  } else {
