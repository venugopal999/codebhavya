(function () {
    ...
    function sequentialSearch(state, target, steps, counters, purpose) {
        ...
        for (let index = 0; index < state.records.length; index += 1) {
            ...
            steps.push(makeStep(
                ...
                "sequential",
                "Compare Key",
                "Compare key " + record.key + " with target " + target + ".",
                "if (current.key == key)",
                Object.assign({}, counters, {
                    activeBlock: block,
                    activeKey: record.key
            ...
            if (record.key === target) {
                steps.push(makeStep(
                    ...
                    "sequential",
                    "Record Found",
                    "Key " + target + " matches record " +
                        recordText(record) + ".",
                    "/* sequential search found */",
                    Object.assign({}, counters, {
                        activeBlock: block,
                        activeKey: target,
                        result: "Found " + record.name +
                            " (" + record.marks + ")"
                    })
                ));
            ...
            if (record.key > target) {
                steps.push(makeStep(
                    ...
                    "sequential",
                    "Stop Early",
                    "Key " + record.key + " is greater than " +
                        target + "; the target cannot appear later.",
    ...
    function buildSteps(
        ...
        if (organization === "sequential") {
            state = buildSequentialBase(
                records,
                steps,
                counters
            );
        } else if (organization === "indexed") {
            state = buildIndexedBase(
                records,
                steps,
                counters
            );
        } else {
            state = buildDirectBase(
                records,
                steps,
                counters
            );
    ...
    function renderBlockedStorage(container, step) {
        if (
            step.organization === "indexed" &&
            step.records.length
        ) {
            const indexStrip = element(
                "div",
                "file-index-strip"
            );

            indexStrip.appendChild(element(
                "strong",
                "file-index-title",
                "Sparse Index"
            ));
    ...
    function renderVisual() {
        ...
        visualizer.organizationValue.textContent =
            definitions[
                step.organization
            ].label;
    ...
    if (tracer.load) {
        ...
        tracer.organization.addEventListener(
            "change",
            function () {
                setTraceExample(
                    tracer.organization.value
