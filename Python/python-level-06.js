(function () {
    ...
    const STORAGE_KEY = "codebhavya.python.level6.practice.v1";
    const MAX_ITERATIONS = 20000;

    const problems = {
        "strings-p1": {
            starter: [
                "# Sample input includes outer spaces:   code bhavya  ",
                "name = input()",
                "clean = name",
                "# Remove outer spaces and apply title case",
                "print(\"Clean =\", clean)"
            ].join("\n"),
            inputs: ["  code bhavya  "],
            expected: "Clean = Code Bhavya"
        },
        "strings-p2": {
            starter: [
                "# Sample inputs: Python and 2",
                "text = input()",
                "index = int(input())",
                "character = \"?\"",
        ...
        "strings-p3": {
            starter: [
                "# Sample inputs: CodeBhavya, 4 and 10",
                "text = input()",
                "start = int(input())",
                "stop = int(input())",
                "part = \"?\"",
                "# Slice text from start to stop",
                "print(\"Slice =\", part)"
            ].join("\n"),
            inputs: ["CodeBhavya", "4", "10"],
            expected: "Slice = Bhavya"
        },
        "strings-p4": {
            starter: [
        ...
        "strings-p5": {
            starter: [
                ...
                "text = input()",
                "vowels = 0",
                "# Traverse the lowercase text and count vowels",
                "print(\"Vowels =\", vowels)"
            ].join("\n"),
            inputs: ["Education"],
            expected: "Vowels = 5"
            ...
            function () {
                ...
                questions.forEach(function (question) {
                    ...
                    feedback.textContent = correct
                        ...
                        : (
                            selected
                                ? "✕ Review the string rule and try again."
                ...
                if (answered < questions.length) {
                    ...
                    result.textContent =
                        "Excellent! " + score + "/" +
                        questions.length +
                        " — your string concepts are clear.";
                } else if (
                    ...
                    result.textContent =
                        "Score: " + score + "/" +
                        questions.length +
                        " — revise indexing, slicing and methods, then try again.";
                }
            }
        );
    ...
    function applyStringMethod(
        ...
        if (typeof base !== "string") {
            throw new Error(
                method + "() requires a string value."
            );
        }

        const parts = argumentsSource.trim() === ""
            ? []
        ...
        if (method === "lstrip" && !args.length) {
            return base.replace(/^\s+/, "");
        }

        if (method === "rstrip" && !args.length) {
            return base.replace(/\s+$/, "");
        }

        if (method === "title" && !args.length) {
        ...
        if (method === "capitalize" && !args.length) {
            return base
                ? base[0].toUpperCase() +
        ...
        if (method === "swapcase" && !args.length) {
        ...
        if (method === "find" && args.length === 1) {
            return base.indexOf(String(args[0]));
        }

        if (method === "replace" && args.length === 2) {
            return base.split(String(args[0]))
                .join(String(args[1]));
        }

        if (method === "startswith" && args.length === 1) {
            return base.startsWith(String(args[0]));
        }

        if (method === "endswith" && args.length === 1) {
            return base.endsWith(String(args[0]));
        }

        if (method === "isalpha" && !args.length) {
            return base.length > 0 && /^[^\W\d_]+$/u.test(base);
        }

        if (method === "isdigit" && !args.length) {
            return base.length > 0 && /^\d+$/.test(base);
        }

        if (method === "isalnum" && !args.length) {
            return base.length > 0 &&
                /^[\p{L}\p{N}]+$/u.test(base);
        }

        if (method === "isspace" && !args.length) {
            return base.length > 0 && /^\s+$/.test(base);
        }

        throw new Error(
            "Unsupported string method or incorrect arguments: " +
            method + "()."
        );
    }
    ...
    function evaluateExpression(
        ...
        if (fString) {
            return fString[2].replace(
                /\{([^{}]+)\}/g,
                function (_, inner) {
                    const formatted = inner.match(
                        /^(.+):\.(\d+)f$/
                    );

                    if (formatted) {
                        const number = evaluateExpression(
                            formatted[1],
                            variables,
                            inputQueue
                        );
        ...
        if (indexMatch) {
            ...
            if (typeof base !== "string") {
                throw new Error(
                    "Indexing in this level requires a string."
                );
            }
        ...
        if (membership) {
            ...
            if (typeof right !== "string") {
                throw new Error(
                    "String membership requires text on the right side."
                );
            }

            const included =
                right.indexOf(String(left)) !== -1;

            return membership.operator.replace(/\s+/g, " ") ===
                "not in"
                ? !included
                : included;
        }
        ...
        if (!/^[0-9+\-*/%().\s<>=!&|]+$/.test(stripped)) {
            throw new Error(
                "This runner supports string operations, arithmetic and logical conditions."
            );
        }
    ...
    function executeStatement(record, context) {
        ...
        throw new Error(
            "Line " + record.line +
            ": use a supported assignment, print(), decision or loop statement."
        );
    }
    ...
    function evaluateIterable(source, context) {
        ...
        throw new Error(
            "A for loop requires a string, sequence or range()."
        );
    }
    ...
    function runStringCode(source, configuredInputs) {
        const context = {
            variables: {},
    ...
    function initializeVisualizer() {
        ...
        const frames = [
            {
                title: "1. Read the Indexed Sequence",
                detail:
                    "Python maps the characters in Python to indexes 0 through 5."
            },
            {
                title: "2. Begin at Index 1",
                detail:
                    "The slice starts with the character y because the start index is included."
            },
            {
                title: "3. Move by a Step of 2",
                detail:
                    "After index 1, Python moves to index 3 and selects h."
            },
            {
                title: "4. Stop Before Index 5",
                detail:
                    "Index 5 is the excluded stop boundary, so its character is not selected."
            },
            {
                title: "5. Build the New String",
                detail:
                    "The selected characters y and h form the new string yh. The original string is unchanged."
            }
        ];
        ...
        const stepText = document.getElementById(
            "pythonStringVisualStep"
        );
        const title = document.getElementById(
            "pythonStringVisualTitle"
        );
        const detail = document.getElementById(
            "pythonStringVisualDetail"
        );
        const dots = document.getElementById(
            "pythonStringVisualDots"
        );
        const progress = document.getElementById(
            "pythonStringVisualProgress"
        );
        ...
        const frames = [
            {
                ...
                output: "",
                note:
                    "Store the four-character string Code."
            },
            {
                line: 1,
                state: {
                ...
                output: "",
                note:
                    "Initialize the vowel counter to zero."
            },
            {
                line: 3,
                state: {
                ...
                output: "",
                note:
                    "c is not in aeiou, so the counter remains zero."
            },
            {
                line: 4,
                state: {
                ...
                output: "",
                note:
                    "o is a vowel, so increment the counter to one."
            },
            {
                line: 3,
                state: {
                ...
                output: "",
                note:
                    "d is not a vowel, so no update occurs."
            },
            {
                line: 4,
                state: {
                ...
                output: "",
                note:
                    "e is a vowel, so increment the counter to two."
            },
            {
                line: 5,
                state: {
                ...
                output: "Vowels = 2",
                note:
                    "Display the completed count. Program execution is complete."
            }
        ];

        const code = document.getElementById(
            "pythonStringTraceCode"
        );
        const note = document.getElementById(
            "pythonStringTraceNote"
        );
        const live = document.getElementById(
            "pythonStringTraceState"
        );
        const output = document.getElementById(
            "pythonStringTraceOutput"
        );
        const status = document.getElementById(
            "pythonStringTraceStatus"
        );
    ...
    window.CodeBhavyaStringRunner = runStringCode;
