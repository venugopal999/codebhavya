(function () {
    ...
    const topic = ["python", "dsa"].includes(requestedTopic) ? requestedTopic : "c";
    const topicLabel = { c: "C", python: "Python", dsa: "DSA" }[topic];
    ...
    const mcqLabels = topic === "python" ? pythonLabels : topic === "dsa" ? dsaLabels : cMcqLabels;
    const codingLabels = topic === "python" ? pythonLabels : topic === "dsa" ? dsaLabels : cCodingLabels;
    const skillOrder = topic === "python" ? pythonOrder : topic === "dsa" ? dsaOrder : cSkillOrder;
    const skillLabels = topic === "python" ? pythonLabels : topic === "dsa" ? dsaLabels : cSkillLabels;
    const codingToSkill = topic === "python" ? pythonCodingToSkill : topic === "dsa" ? dsaIdentityMap : cCodingToSkill;
    const skillToMcq = topic === "python" ? identityMap : topic === "dsa" ? dsaIdentityMap : cSkillToMcq;
    const skillToCoding = topic === "python" ? pythonSkillToCoding : topic === "dsa" ? dsaIdentityMap : cSkillToCoding;
    ...
    function renderSummary(data) {
        ...
        $("codingSolvedNote").textContent = number(summary.total_problems) === 0
            ? `${topicLabel} Coding Arena is the next release`
            : `${number(summary.attempted_problems)} attempted · ${newProblems} new`;
        $("pointsValue").textContent = `${number(summary.points)} points`;
        $("rankValue").textContent = summary.rank ? `Current ${topicLabel} leaderboard rank #${summary.rank}` : "Rank begins after a coding submission";
    ...
    function renderDifficulties(rows) {
        ...
        if (!rows?.length) {
            ...
            note.textContent = `${topicLabel} coding evidence will appear here when its Coding Arena is released.`;
            box.append(note);
            return;
        }
        (rows || []).forEach((row) => {
    ...
    function renderCompanies(rows) {
        ...
        if (!select.options.length) {
            const option = document.createElement("option");
            option.value = "";
            option.textContent = "Coding collections not released";
            ...
            $("companyReadinessMessage").textContent = `${topicLabel} company-linked coding evidence will be added with the Coding Arena.`;
    ...
    function initialize() {
        ...
        if ($("progressProblemLink")) {
            $("progressProblemLink").href = topic === "dsa" ? "practice.html" : `coding.html?topic=${topic}`;
            if (topic === "dsa") $("progressProblemLink").textContent = "DSA Coding Arena comes next →";
        }
        ...
        if ($("navCoding")) $("navCoding").href = topic === "dsa" ? "practice.html" : `coding.html?topic=${topic}`;
