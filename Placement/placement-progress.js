(function () {
    ...
    const topic = ["python", "dsa", "database", "core-cs", "ai-ml"].includes(requestedTopic) ? requestedTopic : "c";
    const topicLabel = { c: "C", python: "Python", dsa: "DSA", database: "Database & SQL", "core-cs": "Core CS", "ai-ml": "AI & ML" }[topic];
    const coreCsTopic = topic === "core-cs";
    const aiMlTopic = topic === "ai-ml";
    const assessmentOnly = coreCsTopic;
    ...
    const mcqLabels = topic === "python" ? pythonLabels : topic === "dsa" ? dsaLabels : topic === "database" ? databaseLabels : coreCsTopic ? coreCsLabels : aiMlTopic ? aiMlLabels : cMcqLabels;
    const codingLabels = topic === "python" ? pythonLabels : topic === "dsa" ? dsaLabels : topic === "database" ? databaseCodingLabels : coreCsTopic ? coreCsLabels : aiMlTopic ? aiMlCodingLabels : cCodingLabels;
    const skillOrder = topic === "python" ? pythonOrder : topic === "dsa" ? dsaOrder : topic === "database" ? databaseOrder : coreCsTopic ? coreCsOrder : aiMlTopic ? aiMlOrder : cSkillOrder;
    const skillLabels = topic === "python" ? pythonLabels : topic === "dsa" ? dsaLabels : topic === "database" ? databaseLabels : coreCsTopic ? coreCsLabels : aiMlTopic ? aiMlLabels : cSkillLabels;
    const codingToSkill = topic === "python" ? pythonCodingToSkill : topic === "dsa" ? dsaCodingToSkill : topic === "database" ? databaseCodingToSkill : coreCsTopic ? coreCsIdentityMap : aiMlTopic ? aiMlCodingToSkill : cCodingToSkill;
    const skillToMcq = topic === "python" ? identityMap : topic === "dsa" ? dsaIdentityMap : topic === "database" ? databaseIdentityMap : coreCsTopic ? coreCsIdentityMap : aiMlTopic ? aiMlIdentityMap : cSkillToMcq;
    const skillToCoding = topic === "python" ? pythonSkillToCoding : topic === "dsa" ? dsaSkillToCoding : topic === "database" ? databaseSkillToCoding : coreCsTopic ? coreCsIdentityMap : aiMlTopic ? aiMlSkillToCoding : cSkillToCoding;
    ...
    function renderSummary(data) {
        ...
        if (assessmentOnly) {
            ...
            $("codingSolvedNote").textContent = number(summary.total_problems) === 0
                ? `Install the ${topic === "database" ? "SQL Query Arena" : topicLabel + " coding"} seed to enable this evidence`
                : `${number(summary.attempted_problems)} attempted · ${newProblems} new`;
            $("pointsValue").textContent = `${number(summary.points)} points`;
            $("rankValue").textContent = summary.rank ? `Current ${topicLabel} leaderboard rank #${summary.rank}` : `Rank begins after a ${topic === "database" ? "SQL query" : "coding"} submission`;
        }
    ...
    function renderDifficulties(rows) {
        ...
        if (!rows?.length) {
            ...
            note.textContent = `${topicLabel} execution evidence will appear after the coding seed is installed and a query is submitted.`;
            box.append(note);
            return;
        }
        (rows || []).forEach((row) => {
    ...
    function renderRecent(rows) {
        ...
        if (!rows?.length) {
            ...
            empty.textContent = assessmentOnly
                ? "No submitted activity yet. Complete a Core CS topic quiz to begin your evidence trail."
        ...
        rows.forEach((row) => {
            ...
            icon.textContent = row.kind === "quiz" ? "Q" : topic === "database" ? "SQL" : aiMlTopic ? "ML" : topic === "python" ? "Py" : topic === "dsa" ? "DS" : "C";
            const copy = document.createElement("div");
            const title = document.createElement("strong");
    ...
    function initialize() {
        ...
        if ($("progressProblemLink")) {
            $("progressProblemLink").href = assessmentOnly ? `mcq-library.html?topic=${topic}` : `coding.html?topic=${topic}`;
        ...
        if ($("navCoding")) {
            $("navCoding").href = assessmentOnly ? "practice.html#core-cs-practice" : `coding.html?topic=${topic}`;
            $("navCoding").textContent = assessmentOnly ? "Practice Hub" : topic === "database" ? "SQL Arena" : aiMlTopic ? "AI/ML Lab" : "Coding Arena";
        }
        ...
        if (topic === "database") {
            ...
            const formula = document.querySelectorAll(".readiness-formula span");
            if (formula[0]) formula[0].innerHTML = "<b>75%</b> MCQ mastery";
            if (formula[1]) formula[1].innerHTML = "<b>15%</b> quiz accuracy";
            ...
            if (routineHeading) routineHeading.textContent = "LEARNING ROUTINE";
            const topicIcon = evidenceCards[2]?.querySelector(".evidence-icon");
            const routineIcon = evidenceCards[3]?.querySelector(".evidence-icon");
            if (topicIcon) topicIcon.textContent = "8";
            if (routineIcon) routineIcon.textContent = "↻";
            const skillEvidence = $("skillProgressGrid")?.closest("section")?.querySelector("header > span");
            if (skillEvidence) skillEvidence.textContent = "MCQ + quiz evidence";
            const intro = document.querySelector(".progress-intro p");
            if (intro) intro.textContent = "Every number below comes from completed Core CS assessments and learning activity. Strengthen the weakest evidence first.";
            const loadingCopy = $("progressLoading")?.querySelector("p");
            if (loadingCopy) loadingCopy.textContent = "Combining MCQ mastery, quiz accuracy and learning consistency.";
