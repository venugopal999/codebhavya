(function () {
    ...
    const mcqLabels = topic === "python" ? pythonLabels : topic === "dsa" ? dsaLabels : topic === "database" ? databaseLabels : aiMlTopic ? aiMlLabels : cMcqLabels;
    const codingLabels = topic === "python" ? pythonLabels : topic === "dsa" ? dsaLabels : topic === "database" ? databaseCodingLabels : aiMlTopic ? aiMlCodingLabels : cCodingLabels;
    const skillOrder = topic === "python" ? pythonOrder : topic === "dsa" ? dsaOrder : topic === "database" ? databaseOrder : aiMlTopic ? aiMlOrder : cSkillOrder;
    const skillLabels = topic === "python" ? pythonLabels : topic === "dsa" ? dsaLabels : topic === "database" ? databaseLabels : aiMlTopic ? aiMlLabels : cSkillLabels;
    const codingToSkill = topic === "python" ? pythonCodingToSkill : topic === "dsa" ? dsaCodingToSkill : topic === "database" ? databaseCodingToSkill : aiMlTopic ? aiMlCodingToSkill : cCodingToSkill;
    const skillToMcq = topic === "python" ? identityMap : topic === "dsa" ? dsaIdentityMap : topic === "database" ? databaseIdentityMap : aiMlTopic ? aiMlIdentityMap : cSkillToMcq;
    const skillToCoding = topic === "python" ? pythonSkillToCoding : topic === "dsa" ? dsaSkillToCoding : topic === "database" ? databaseSkillToCoding : aiMlTopic ? aiMlSkillToCoding : cSkillToCoding;
    ...
    function readinessText(value) {
        ...
        return ["Build the foundation.", topic === "database" ? "Complete your first topic quiz and SQL query challenge to start measuring your progress." : "Complete your first topic quiz and coding challenge to start measuring your progress."];
    }
    ...
    function renderRecent(rows) {
        ...
        if (!rows?.length) {
            ...
            empty.textContent = `No submitted activity yet. Complete a quiz or submit a ${topic === "database" ? "SQL query" : "coding solution"} to begin your evidence trail.`;
            box.append(empty);
            return;
        }
    ...
    function initialize() {
        ...
        if ($("progressProblemLink")) {
            $("progressProblemLink").href = `coding.html?topic=${topic}`;
        }
        ...
        if ($("navCoding")) {
            ...
            $("navCoding").textContent = topic === "database" ? "SQL Arena" : aiMlTopic ? "AI/ML Lab" : "Coding Arena";
        }
