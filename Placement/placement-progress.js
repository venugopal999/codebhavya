(function () {
    ...
    const topic = ["python", "dsa", "database"].includes(requestedTopic) ? requestedTopic : "c";
    const topicLabel = { c: "C", python: "Python", dsa: "DSA", database: "Database & SQL" }[topic];
    const hasCodingArena = topic !== "database";
    ...
    const mcqLabels = topic === "python" ? pythonLabels : topic === "dsa" ? dsaLabels : topic === "database" ? databaseLabels : cMcqLabels;
    const codingLabels = topic === "python" ? pythonLabels : topic === "dsa" ? dsaLabels : topic === "database" ? databaseLabels : cCodingLabels;
    const skillOrder = topic === "python" ? pythonOrder : topic === "dsa" ? dsaOrder : topic === "database" ? databaseOrder : cSkillOrder;
    const skillLabels = topic === "python" ? pythonLabels : topic === "dsa" ? dsaLabels : topic === "database" ? databaseLabels : cSkillLabels;
    const codingToSkill = topic === "python" ? pythonCodingToSkill : topic === "dsa" ? dsaCodingToSkill : topic === "database" ? databaseIdentityMap : cCodingToSkill;
    const skillToMcq = topic === "python" ? identityMap : topic === "dsa" ? dsaIdentityMap : topic === "database" ? databaseIdentityMap : cSkillToMcq;
    const skillToCoding = topic === "python" ? pythonSkillToCoding : topic === "dsa" ? dsaSkillToCoding : topic === "database" ? databaseIdentityMap : cSkillToCoding;
    ...
    function readinessText(value) {
        ...
        return ["Build the foundation.", hasCodingArena ? "Complete your first topic quiz and coding challenge to start measuring your progress." : "Complete your first Database & SQL topic quiz to start measuring your progress."];
    }
    ...
    function renderSummary(data) {
        ...
        $("codingSolvedNote").textContent = number(summary.total_problems) === 0
            ? (topic === "database" ? "The executable SQL Query Arena is the next release" : `Install the ${topicLabel} coding seed to enable this evidence`)
            : `${number(summary.attempted_problems)} attempted · ${newProblems} new`;
        $("pointsValue").textContent = hasCodingArena ? `${number(summary.points)} points` : `${number(summary.quiz_sessions)} submitted quizzes`;
        $("rankValue").textContent = hasCodingArena ? (summary.rank ? `Current ${topicLabel} leaderboard rank #${summary.rank}` : "Rank begins after a coding submission") : "SQL points and rank begin with the Query Arena";
    ...
    function renderActions(skills, data) {
        ...
        box.replaceChildren(
            makeAction(1, "REVISE", `Strengthen ${first.label}`, "Read five explained MCQs and mark only the concepts you can recall.", `mcq-library.html?topic=${topic}&group=${encodeURIComponent(skillToMcq[first.key])}`),
            makeAction(2, "CODE", `Solve ${second.label}`, `Continue with ${codingDifficulty} problems and submit against all protected tests.`, `coding.html?topic=${topic}&mode=topic&group=${encodeURIComponent(skillToCoding[second.key])}&difficulty=${codingDifficulty}`),
    ...
    function renderDifficulties(rows) {
        ...
        if (!rows?.length) {
            ...
            note.textContent = topic === "database" ? "The SQL Query Arena will add executable query depth here in the next release." : `${topicLabel} coding evidence will appear here when its Coding Arena is released.`;
            box.append(note);
            return;
        }
        (rows || []).forEach((row) => {
    ...
    function renderCompanies(rows) {
        ...
        if (!select.options.length) {
            const option = document.createElement("option");
            option.value = "";
            option.textContent = topic === "database" ? "SQL collections coming next" : "Coding collections not released";
            ...
            $("companyReadinessMessage").textContent = topic === "database" ? "Use Preparation Track quizzes now; company-linked SQL tasks arrive with the Query Arena." : `${topicLabel} company-linked coding evidence will be added with the Coding Arena.`;
    ...
    function renderPlan(skills, data) {
        ...
        box.replaceChildren(
            planItem("1", "READ", weak[0].label, "Review five explained MCQs.", `mcq-library.html?topic=${topic}&group=${encodeURIComponent(skillToMcq[weak[0].key])}`),
            planItem("2", "CODE", weak[0].label, "Solve two beginner problems.", `coding.html?topic=${topic}&mode=topic&group=${encodeURIComponent(skillToCoding[weak[0].key])}&difficulty=beginner`),
            ...
            planItem("5", "CODE", weak[1].label, "Submit two coding solutions.", `coding.html?topic=${topic}&mode=topic&group=${encodeURIComponent(skillToCoding[weak[1].key])}`),
    ...
    function renderRecent(rows) {
        ...
        if (!rows?.length) {
            ...
            empty.textContent = hasCodingArena ? "No submitted activity yet. Complete a quiz or submit a coding solution to begin your evidence trail." : "No submitted activity yet. Complete a Database & SQL quiz to begin your evidence trail.";
            box.append(empty);
            return;
        }
    ...
    function initialize() {
        ...
        if ($("progressProblemLink")) {
            $("progressProblemLink").href = hasCodingArena ? `coding.html?topic=${topic}` : "practice.html#database-practice";
            if (!hasCodingArena) $("progressProblemLink").textContent = "SQL Query Arena comes next →";
        }
        ...
        if ($("navCoding")) {
            $("navCoding").href = hasCodingArena ? `coding.html?topic=${topic}` : "practice.html#database-practice";
            $("navCoding").textContent = hasCodingArena ? "Coding Arena" : "Practice Hub";
        }
        if ($("navProgress")) $("navProgress").href = `progress.html?topic=${topic}`;
        if (!hasCodingArena) {
            const formula = document.querySelectorAll(".readiness-formula span");
            if (formula.length >= 4) {
                formula[0].innerHTML = "<b>75%</b> MCQ mastery";
                formula[1].innerHTML = "<b>15%</b> quiz accuracy";
                formula[2].innerHTML = "<b>10%</b> consistency";
                formula[3].textContent = "SQL execution joins next";
            }
            const codingHeading = $("codingSolvedValue")?.closest("article")?.querySelector("small");
            if (codingHeading) codingHeading.textContent = "SQL QUERY ARENA";
            const pointsHeading = $("pointsValue")?.closest("article")?.querySelector("small");
            if (pointsHeading) pointsHeading.textContent = "ASSESSMENT EVIDENCE";
            const skillEvidence = $("skillProgressGrid")?.closest("section")?.querySelector("header > span");
            if (skillEvidence) skillEvidence.textContent = "MCQ assessment evidence";
        }
