(function () {
    ...
    const topic = ["python", "dsa", "database", "ai-ml"].includes(requestedTopic) ? requestedTopic : "c";
    const topicLabel = { c: "C", python: "Python", dsa: "DSA", database: "Database & SQL", "ai-ml": "AI & ML" }[topic];
    const assessmentOnly = topic === "ai-ml";
    ...
    const mcqLabels = topic === "python" ? pythonLabels : topic === "dsa" ? dsaLabels : topic === "database" ? databaseLabels : assessmentOnly ? aiMlLabels : cMcqLabels;
    const codingLabels = topic === "python" ? pythonLabels : topic === "dsa" ? dsaLabels : topic === "database" ? databaseCodingLabels : assessmentOnly ? aiMlLabels : cCodingLabels;
    const skillOrder = topic === "python" ? pythonOrder : topic === "dsa" ? dsaOrder : topic === "database" ? databaseOrder : assessmentOnly ? aiMlOrder : cSkillOrder;
    const skillLabels = topic === "python" ? pythonLabels : topic === "dsa" ? dsaLabels : topic === "database" ? databaseLabels : assessmentOnly ? aiMlLabels : cSkillLabels;
    const codingToSkill = topic === "python" ? pythonCodingToSkill : topic === "dsa" ? dsaCodingToSkill : topic === "database" ? databaseCodingToSkill : assessmentOnly ? aiMlIdentityMap : cCodingToSkill;
    const skillToMcq = topic === "python" ? identityMap : topic === "dsa" ? dsaIdentityMap : topic === "database" ? databaseIdentityMap : assessmentOnly ? aiMlIdentityMap : cSkillToMcq;
    const skillToCoding = topic === "python" ? pythonSkillToCoding : topic === "dsa" ? dsaSkillToCoding : topic === "database" ? databaseSkillToCoding : assessmentOnly ? aiMlIdentityMap : cSkillToCoding;
    ...
    function renderRecent(rows) {
        ...
        if (!rows?.length) {
            ...
            empty.textContent = assessmentOnly
                ? "No submitted activity yet. Complete an AI & ML topic quiz to begin your evidence trail."
        ...
        rows.forEach((row) => {
            ...
            icon.textContent = row.kind === "quiz" ? "Q" : topic === "database" ? "SQL" : "C";
            const copy = document.createElement("div");
            const title = document.createElement("strong");
    ...
    function initialize() {
        ...
        if ($("navCoding")) {
            $("navCoding").href = assessmentOnly ? "practice.html#ai-ml-practice" : `coding.html?topic=${topic}`;
            $("navCoding").textContent = assessmentOnly ? "Practice Hub" : topic === "database" ? "SQL Arena" : "Coding Arena";
        }
        ...
        if (topic === "database") {
            ...
            if (intro) intro.textContent = "Every number below comes from completed AI & ML assessments and learning activity. Strengthen the weakest evidence first.";
