(function () {
    ...
    const modeContent = {
        topic: {
            help: "Choose one C topic and strengthen it from fundamentals through advanced reasoning.",
            eyebrow: "TOPIC MASTERY",
            title: "Build complete confidence in one C topic.",
        ...
        track: {
            ...
            text: "Each track brings together the C skills commonly emphasised in that hiring pattern.",
        ...
        mock: {
            ...
            text: "The mock draws from multiple C topics and keeps all explanations hidden until submission.",
    ...
    function scopeLabel(values) {
        if (values.mode === "topic") return topicGroupLabels[values.topicGroup] || "Selected C topic";
        if (values.mode === "track") return targetLabels[values.target] || "Selected preparation track";
        return values.target === "all" ? "Complete C Placement Mock" : (targetLabels[values.target] + " Mock");
    }
    ...
    async function refreshAvailability() {
        ...
        try {
            ...
            $("quizAvailability").textContent = schemaMissing
                ? "Run the latest placement-v2-schema.sql and C question seed to enable learning modes."
    ...
    async function initialize() {
        const requested = new URLSearchParams(location.search).get("topic") || "c";
        topic = topicLabels[requested] ? requested : "c";
        const label = topicLabels[topic];
        $("builderTitle").textContent = label + " Placement Quiz";
        $("topicCrumb").textContent = label + " Quiz";
        document.title = label + " Placement Quiz | CodeBhavya";
