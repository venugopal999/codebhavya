(function () {
    ...
    const topic = pageParameters.get("topic") === "python" ? "python" : "c";
    ...
    function initializeEditor() {
        ...
        if (window.CodeMirror) {
            editor = window.CodeMirror.fromTextArea(textarea, {
                mode: topic === "python" ? "python" : "text/x-csrc",
                lineNumbers: true,
                matchBrackets: true,
            ...
            if (typeof editor.addOverlay === "function") {
                const builtins = topic === "python"
                    ? /^(?:print|input|len|range|enumerate|zip|map|filter|sum|min|max|sorted|reversed|set|dict|list|tuple|str|int|float|bool|abs|round|open|super|isinstance)\b/
                    : /^(?:printf|scanf|fgets|puts|putchar|getchar|strlen|strcmp|strcpy|strncpy|malloc|calloc|realloc|free|abs|labs|sqrt|pow|tolower|toupper|isdigit|isalpha|qsort)\b/;
                editor.addOverlay({
                    token(stream) {
    ...
    function draftKey() {
        ...
        return `codebhavya-${topic}-draft:${user?.id || "guest"}:${active.slug}`;
    }
    ...
    async function loadRequestedProblem() {
        ...
        if (!slug || !/^[a-z0-9-]+$/i.test(slug)) {
            showWorkspace(false);
            setEmpty(`Choose a ${topicLabel} problem first`, "Return to the Problem Library and select the challenge you want to solve.");
            return;
        }
        if (!client) {
            showWorkspace(false);
    ...
    function selectProblem(problem) {
        try {
            ...
            $("problemTitle").textContent = problem.title || `${topicLabel} problem`;
    ...
    async function judge(mode) {
        ...
        if (code.trim().length < 20) {
            setStatus(`Write a complete ${topicLabel} program before running.`, "error");
            return;
        }
    ...
    async function initialize() {
        $("arenaHeading").textContent = `${topicLabel} Coding Arena`;
        $("backToProblems").textContent = `${topicLabel} Problem Library`;
        $("leaderboardEyebrow").textContent = `${topicLabel.toUpperCase()} PLACEMENT RANKING`;
        $("editorFilename").textContent = topic === "python" ? "solution.py" : "solution.c";
        $("codeEditor").setAttribute("aria-label", `${topicLabel} source code`);
        $("codingLanguage").replaceChildren(new Option(topic === "python" ? "Python 3" : "C (GCC)", topic));
