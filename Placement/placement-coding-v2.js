(function () {
    ...
    const topic = ["c", "python", "dsa"].includes(requestedTopic) ? requestedTopic : "c";
    const topicLabel = topic === "python" ? "Python" : topic === "dsa" ? "DSA" : "C";
    const targetLabels = {
    ...
    let draftTimer = null;
    let runtimeLanguage = topic === "python" ? "python" : "c";
    ...
    function initializeEditor() {
        ...
        if (window.CodeMirror) {
            editor = window.CodeMirror.fromTextArea(textarea, {
                mode: runtimeLanguage === "python" ? "python" : "text/x-csrc",
                lineNumbers: true,
                matchBrackets: true,
    ...
    function updateLanguageUI() {
        const languageLabel = runtimeLanguage === "python" ? "Python" : "C";
        $("editorFilename").textContent = runtimeLanguage === "python" ? "solution.py" : "solution.c";
        $("codeEditor").setAttribute("aria-label", `${languageLabel} source code for ${topicLabel} practice`);
        if (editor) {
            editor.setOption("mode", runtimeLanguage === "python" ? "python" : "text/x-csrc");
            window.setTimeout(() => editor.refresh(), 0);
        }
        if (topic === "dsa") {
    ...
    async function judge(mode) {
        ...
        if (code.trim().length < 20) {
            setStatus(`Write a complete ${runtimeLanguage === "python" ? "Python" : "C"} program before running.`, "error");
            return;
        }
